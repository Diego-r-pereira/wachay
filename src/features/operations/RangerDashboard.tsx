...
import { RangerReportWizard } from '../reports/RangerReportWizard';
import { CitizenApprovalMatrix } from '../admin/CitizenApprovalMatrix';
import { MobileNetFireClassifier } from './MobileNetFireClassifier';
import { WeatherRiskWidget } from './WeatherRiskWidget';
import { RiskProjectionsWidget } from './RiskProjectionsWidget';
import { ReportsExportModal } from '../reports/ReportsExportModal';
import { IncidentReport } from '../../types';
import { Shield, Flame, PlusCircle, Clock, Cpu, Compass, FileText, Trees } from 'lucide-react';
...
export const RangerDashboard: React.FC<RangerDashboardProps> = ({
  reports,
  pendingReports,
  onRefreshReports,
}) => {
  const [activeTab, setActiveTab] = useState<'focos' | 'manual' | 'pendientes' | 'ia' | 'proyecciones'>('focos');
  const [isExportModalOpen, setIsExportModalOpen] = useState(false);

  const activeFiresCount = reports.filter((r) => r.status === 'Second_State').length;
  ...
  const RANGER_TABS = [
    ...
    {
      id: 'proyecciones' as const,
      label: 'Proyecciones y Riesgo',
      icon: Compass,
      isAlertCategory: false,
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 relative z-10">
        ...
        >
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 font-mono text-[10px] font-extrabold uppercase border border-emerald-200 dark:border-emerald-700/50">
                  SERNAP • Puesto de Mando y Campo Tunari
                </span>
              </div>
              <h1 className="text-2xl sm:text-3xl font-black font-heading text-slate-900 dark:text-white">
                Panel de Operaciones de Guardaparques
              </h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Monitoreo satelital, registro directo de patrullaje, clasificación con visión artificial y despacho de cuadrillas.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            {activeFiresCount > 0 ? (
              <span className="px-3.5 py-2.5 rounded-2xl bg-rose-100 dark:bg-rose-950/60 border border-rose-300 dark:border-rose-800 text-rose-900 dark:text-rose-300 text-xs font-black font-mono flex items-center gap-2 shadow-xs">
                <Flame className="w-4 h-4 text-rose-600 animate-pulse" />
                <span>{activeFiresCount} FOCOS ACTIVOS</span>
              </span>
            ) : (
              <span className="px-3.5 py-2.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-bold font-mono flex items-center gap-2">
                <Trees className="w-4 h-4 text-emerald-600" />
                <span>ZONA TUNARI SEGURA</span>
              </span>
            )}
        ...
        {/* ========================================================================= */}
        {/* LUXURY SEGMENTED NAVIGATION DOCK FOR RANGERS */}
        {/* ========================================================================= */}
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'proyecciones' && (
            <motion.div
              key="ranger-tab-proyecciones"
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -12 }}
              transition={{ duration: 0.2 }}
            >
              <RiskProjectionsWidget existingReports={reports} />
            </motion.div>
          )}
        </AnimatePresence>
